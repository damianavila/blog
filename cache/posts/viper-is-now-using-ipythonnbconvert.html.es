...
<p>With the last release, <strong>IPython</strong> support this kind of exportation using the well designed and easily extensible <code>IPython.nbconvert</code> library. Now, it is easier to incorporate the IPython machinery inside vIPer and support the exportation to several formats.
<!-- TEASER_END --></p>
...
</code></pre><p><strong>Note</strong>: yes, I have to deal with the path in a better way, but I am too lazy to do it now ;-)</p>
...
<pre><code>class Converter:
    def __init__(self, exporter, extension, container):
        ...
        if len(self.container.servePool) == 1:
            ...
            pass
</code></pre><p>It is important to highlight that I needed to call the <code>start()</code> method to <em>run</em> the <code>run()</code> method from the <code>ServeThread</code> class.</p>
