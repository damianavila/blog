...
<p>You are in vacation time but you want to blog something nice to share with your friends.</p>
<p>And your blog is powered by <strong>Nikola</strong>.</p>
<p>And you are a <strong>Jupyter Notebook</strong> user.</p>
<p>But you don't have your laptop with you, because you are on <em>vacation</em> time, remember? ;-)</p>
<p>But you still have your phone and some connectivity.</p>
<p>What do you think if I say that you have a complete workflow to write your blogpost, build the site and deploy it just using your phone? But without using computational resources from it (that would be also interesting, btw).</p>
<p>Don't you believe me? Just read this post and I will show you how you can make this possible... and fun!
<!-- TEASER_END --></p>
<p><strong>NOTE</strong>: This post is long but I promise will be interesting! Also, there are multiple cells generating a <em>lot</em> of output, remember that you can hide the output clicking on <em>"Click me to hide the output"</em> button at the bottom/right conner of each cell.</p>
...
<h2 id="Method">Method<a class="anchor-link" href="#Method">&#182;</a></h2><p>Let's summarize the whole idea:</p>
<blockquote><p>We launch a <em>binder</em> instance from a <strong>Github</strong> repo containing my <strong>Nikola</strong>-powered blog site and write some content into a new blog post (using the <strong>Jupyter Notebook</strong>), then I rebuild the site using <strong>Nikola</strong> (already installed in my <em>binder</em> instance) and finally push the updated site to <strong>Github</strong>'s <em>gh-pages</em> to finally publish the new content (from my <em>binder</em> instance as well).</p>
</blockquote>
<p>Sounds good, isn't it?</p>
<h3 id="Create-a-binder-for-my-blog-site">Create a binder for my blog site<a class="anchor-link" href="#Create-a-binder-for-my-blog-site">&#182;</a></h3><p>Do you know <a href="https://mybinder.org/"><strong>Binder</strong></a>, right? In simple words it <em>Turn a GitHub repo into a collection of interactive notebooks</em>. You can find more info about <strong>Binder</strong> at <a href="https://mybinder.readthedocs.io/en/latest/">https://mybinder.readthedocs.io/en/latest/</a>.
For the purpose of this post (and the next ones), I just created a new binder filling my blog site <strong>Github</strong> repo url into the URL field at mybinder.org (and the path to a specific notebook file) and I got a new url to access my <em>binder</em> instance at <a href="https://mybinder.org/v2/gh/damianavila/damian_blog/master?filepath=Start.ipynb">https://mybinder.org/v2/gh/damianavila/damian_blog/master?filepath=Start.ipynb</a></p>
<blockquote><p>In fact, I am writing this blog post in a binder instance (I love to do meta stuff ;-)</p>
</blockquote>
<p>That url pointed me to a <code>Start.ipynb</code> notebook file where I have some cells to create a new post with some <strong>Nikola</strong> tags. I run those cells and I have a new <code>ipynb</code> file under the <code>posts</code> directory, in this case: <code>binder-+-nikola-+-jupyter-+-github-blogging-resourceless.ipynb</code> where I can write my new content.</p>
<h3 id="Write-the-new-content">Write the new content<a class="anchor-link" href="#Write-the-new-content">&#182;</a></h3><p>OK, this is one is easy, it is this very same post ;-) as you probably expected.</p>
<p>BTW, one interesting note, since your binder is using <code>git</code> for internal stuff, you have <code>git</code> installed and ready to do your stuff as well. For instance, if my connection is flaky and given that my <em>binder</em> in <em>transient</em>. I can commit the work done so far. Just save the notebook file and run:</p>
...
<div class="inner_cell">
<div class="text_cell_render border-box-sizing rendered_html">
<p>OK, now you need to push this content and if you try to <code>git push</code> it, it will fail unless you have permissions to push to your own repo. To achieve that from a <em>binder</em> instance you need to create a <em>personal access token</em> from Github: <a href="https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line/">https://help.github.com/articles/creating-a-personal-access-token-for-the-command-line/</a>. And now you will be able to push your stuff.</p>
<p>Note this <em>personal access token</em> will be useful for further pushes down the road, so keep it close.</p>
<p>Also, we need a <em>terminal</em> to be able to provide the <em>username</em> and the <em>token</em>. And the <strong>Jupyter</strong> interface give us that terminal pretty easily, you just go to the /tree (dashboard) and start a terminal from the dropdown menu (I can probably write a little extension to perform this step from the notebook itself... but going to the terminal is pretty easy and quick).</p>
<p><img src="http://www.damian.oquanta.info/galleries/binder/terminal.png" alt=""></p>
<p>Now your repository is updated, so next time you launch a binder from it, it will contain your new post, meaning you can write your stuff iteratevely even in a <em>transient</em> platform, sweet!</p>
<h3 id="Rebuild-your-site-with-Nikola">Rebuild your site with Nikola<a class="anchor-link" href="#Rebuild-your-site-with-Nikola">&#182;</a></h3><p>When your post is ready, you need to rebuild the site using <strong>Nikola</strong>, but how <strong>Nikola</strong> end up in my <em>binder</em> instance?
Easy as well, you just need to provide a <code>requirements.txt</code> file with the dependencies you need and those ones will be included in your <em>binder</em> instance: <a href="https://mybinder.readthedocs.io/en/latest/using.html#simple-python-dependencies">https://mybinder.readthedocs.io/en/latest/using.html#simple-python-dependencies</a></p>
<p>In my case, the <code>requirement.txt</code> file is pretty easy: <a href="https://github.com/damianavila/damian_blog/blob/master/requirements.txt">https://github.com/damianavila/damian_blog/blob/master/requirements.txt</a>. Because all the other pieces I need are already included by default: <strong>Jupyter</strong> stuff and <code>git</code>.</p>
...
<div class="output_subarea output_stream output_stdout output_text">
<pre><span class="ansi-yellow-intense-fg ansi-bold">[2017-12-17T22:28:50Z] WARNING: Nikola: The RSS_LINKS_APPEND_QUERY option is deprecated, use FEED_LINKS_APPEND_QUERY instead.</span>
<span class="ansi-yellow-intense-fg ansi-bold">[2017-12-17T22:28:50Z] WARNING: Nikola: The RSS_READ_MORE_LINK option is deprecated, use FEED_READ_MORE_LINK instead.</span>
<span class="ansi-yellow-intense-fg ansi-bold">[2017-12-17T22:28:50Z] WARNING: Nikola: Could not guess locale for language es, using locale en_US.UTF-8</span>
<span class="ansi-yellow-intense-fg ansi-bold">[2017-12-17T22:28:50Z] WARNING: Nikola: Please fix your OS locale configuration or use the LOCALES option in conf.py to specify your preferred locale.</span>
<span class="ansi-yellow-intense-fg ansi-bold">[2017-12-17T22:28:50Z] WARNING: Nikola: Make sure to use an UTF-8 locale to ensure Unicode support.</span>
Scanning posts......done!
...
<div class="inner_cell">
<div class="text_cell_render border-box-sizing rendered_html">
<p>Now you have your site rebuilt, let's commit the changes so we don't loss the new content.</p>
...
Changes to be committed:
  (use &#34;git reset HEAD &lt;file&gt;...&#34; to unstage)
	...
	<span class="ansi-green-fg">modified:   ../.doit.db.bak</span>
	<span class="ansi-green-fg">modified:   ../.doit.db.dat</span>
	<span class="ansi-green-fg">modified:   ../.doit.db.dir</span>
	<span class="ansi-green-fg">modified:   ../.ipynb_checkpoints/Start-checkpoint.ipynb</span>
	<span class="ansi-green-fg">modified:   ../Start.ipynb</span>
	<span class="ansi-green-fg">modified:   ../__pycache__/conf.cpython-35.pyc</span>
	<span class="ansi-green-fg">modified:   ../cache/jinja/__jinja2_1424a9c95f5ec8e60d413c42a25bc77a4b63ee8b.cache</span>
	<span class="ansi-green-fg">modified:   ../cache/jinja/__jinja2_42065261e76b7ac61d15945dd99b46d11ef2d31d.cache</span>
	<span class="ansi-green-fg">modified:   ../cache/jinja/__jinja2_5e46cfeeff2a14274a55bed284465c9788e60bfe.cache</span>
	<span class="ansi-green-fg">modified:   ../cache/jinja/__jinja2_bf78ea284af06dd653f1623611857af052bad3c2.cache</span>
	<span class="ansi-green-fg">modified:   ../cache/jinja/__jinja2_d18a7e23288807856bde0b72920980e5c037e91e.cache</span>
	<span class="ansi-green-fg">modified:   ../output/2017/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/archive.html</span>
	<span class="ansi-green-fg">modified:   ../output/categories/aikido.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/base16.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/blog.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/conda.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/config.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/connectivity.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/css.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/daniela.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/extension.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/facu.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/facundo.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/family.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/first_post.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/flood.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/format.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/gh-pages.html</span>
	<span class="ansi-green-fg">modified:   ../output/categories/gh-pages.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/git.html</span>
	<span class="ansi-green-fg">modified:   ../output/categories/git.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/categories/ipyscript.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/ipython.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/ipytmpl.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/javascript.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/jinja.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/jupyter.html</span>
	<span class="ansi-green-fg">modified:   ../output/categories/jupyter.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/jupyterlab.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/less.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/life.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/linux.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/local.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/magic.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/minimap.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/mipyex.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/nbconvert.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/nbviewer.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/nikola.html</span>
	<span class="ansi-green-fg">modified:   ../output/categories/nikola.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/opensource.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/osx.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/pdf.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/performance.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/pinned.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/pip.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/politics.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/pyconar.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/python.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/reveal.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/rise.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/scipycon.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/slideshow.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/spellcheck.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/talks.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/theme.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/themes.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/threads.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/toggle.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/trading.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/tutorials.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/twitter.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/typescript.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/videos.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/viper.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/win.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/workflow.html</span>
	<span class="ansi-green-fg">modified:   ../output/categories/workflow.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/zen.xml</span>
	<span class="ansi-green-fg">modified:   ../output/categories/zenmode.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/2012/06/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2012/09/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2012/10/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2012/11/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2012/12/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2012/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2013/01/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2013/02/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2013/04/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2013/05/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2013/08/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2013/09/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2013/10/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2013/11/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2013/12/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2013/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2014/03/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2014/04/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2014/05/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2014/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2016/05/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2016/06/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2016/08/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2016/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2017/06/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2017/07/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2017/11/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/2017/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/archive.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/aikido.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/base16.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/blog.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/conda.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/config.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/connectivity.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/css.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/daniela.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/extension.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/facu.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/facundo.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/family.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/first_post.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/flood.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/format.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/gh-pages.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/gh-pages.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/git.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/git.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/ipyscript.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/ipython.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/ipytmpl.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/javascript.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/jinja.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/jupyter.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/jupyter.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/jupyterlab.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/less.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/life.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/linux.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/local.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/magic.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/minimap.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/mipyex.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/nbconvert.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/nbviewer.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/nikola.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/nikola.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/opensource.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/osx.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/pdf.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/performance.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/pinned.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/pip.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/politics.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/pyconar.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/python.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/reveal.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/rise.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/scipycon.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/slideshow.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/spellcheck.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/talks.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/theme.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/themes.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/threads.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/toggle.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/trading.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/tutorials.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/twitter.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/typescript.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/videos.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/viper.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/win.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/workflow.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/workflow.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/zen.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/categories/zenmode.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/galleries/Facu_RN/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/galleries/Facu_RN/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/galleries/Inundacion/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/galleries/bokehmagic/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/galleries/bokehmagic/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/galleries/deploy/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/galleries/loadwrite/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/galleries/loadwrite/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/galleries/rise/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/galleries/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/galleries/spellchecker/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/galleries/spellchecker/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/galleries/trades/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/galleries/twitter/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/galleries/zenmode/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/galleries/zenmode/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/es/index-5.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/posts/rise-510-is-out.html</span>
	<span class="ansi-green-fg">modified:   ../output/es/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/galleries/Facu_RN/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/galleries/Facu_RN/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/galleries/Inundacion/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/galleries/bokehmagic/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/galleries/bokehmagic/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/galleries/deploy/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/galleries/loadwrite/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/galleries/loadwrite/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/galleries/rise/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/galleries/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/galleries/spellchecker/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/galleries/spellchecker/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/galleries/trades/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/galleries/twitter/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/galleries/zenmode/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/galleries/zenmode/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/index-5.html</span>
	<span class="ansi-green-fg">modified:   ../output/index.html</span>
	<span class="ansi-green-fg">modified:   ../output/posts/rise-510-is-out.html</span>
	<span class="ansi-green-fg">modified:   ../output/rss.xml</span>
	<span class="ansi-green-fg">modified:   ../output/sitemap.xml</span>
	<span class="ansi-green-fg">modified:   ../output/sitemapindex.xml</span>
	<span class="ansi-green-fg">modified:   ../plugins/upgrade_metadata/__pycache__/upgrade_metadata.cpython-35.pyc</span>
	<span class="ansi-green-fg">modified:   binder-+-nikola-+-jupyter-+-github-blogging-resourceless.ipynb</span>
...
Untracked files:
  (use &#34;git add &lt;file&gt;...&#34; to include in what will be committed)
	...
	<span class="ansi-red-fg">../.local/</span>
	<span class="ansi-red-fg">../.profile</span>
	<span class="ansi-red-fg">../cache/jinja/__jinja2_1f7b67a24ba82817190ca9329b4e70f5b83d2aa8.cache</span>
	<span class="ansi-red-fg">../cache/jinja/__jinja2_29f4b4300680b610c26f01cd4b344c99659e2ae7.cache</span>
	<span class="ansi-red-fg">../cache/jinja/__jinja2_45ac9368227a101d2f06edaefd1c3a89262c46de.cache</span>
	<span class="ansi-red-fg">../cache/jinja/__jinja2_57b64e8d8c69c8a0309a317dada665de6a09131f.cache</span>
	<span class="ansi-red-fg">../cache/jinja/__jinja2_5e39743f43676d71d1fbf5a3f5e6c7bcd0553d17.cache</span>
	<span class="ansi-red-fg">../cache/jinja/__jinja2_697232dc3c14dcf966cb673a46734538fbdcfaff.cache</span>
	<span class="ansi-red-fg">../cache/jinja/__jinja2_7569c06e91d40657d212e7347a7c0ba9682aecbe.cache</span>
	<span class="ansi-red-fg">../cache/jinja/__jinja2_85b3b28c2af8137138202b66ddb1de1e27daa0cb.cache</span>
	<span class="ansi-red-fg">../cache/jinja/__jinja2_8a9ea377d370579847f6f32b831d680ddc1ceac5.cache</span>
	<span class="ansi-red-fg">../cache/jinja/__jinja2_91ca140498c6809d832a9440553bc6cb8df50df9.cache</span>
	<span class="ansi-red-fg">../cache/jinja/__jinja2_a77ac1ecf914de6946a75d0d3591cbafb1effc32.cache</span>
	<span class="ansi-red-fg">../cache/jinja/__jinja2_abbd706e988d6c55c3dd152bf5ed1d756595f719.cache</span>
	<span class="ansi-red-fg">../cache/jinja/__jinja2_b04036e3f5288f19927f5dc3b3651ec8b42f8e2c.cache</span>
	<span class="ansi-red-fg">../cache/jinja/__jinja2_b807b7a7fd71f025befffb377825b9c7576fe519.cache</span>
	<span class="ansi-red-fg">../cache/jinja/__jinja2_b94abb01e952cf6371a6460a7adcea55c54b074e.cache</span>
	<span class="ansi-red-fg">../cache/jinja/__jinja2_c28a0a887f59ee48243095e6aad33a6c2aa828d2.cache</span>
	<span class="ansi-red-fg">../cache/jinja/__jinja2_dcbc01075586f1698433d8ae5d61d6fc19df7f17.cache</span>
	<span class="ansi-red-fg">../cache/jinja/__jinja2_e609ceaaa3f2e31193a281ee74d8fd5b52956a69.cache</span>
	<span class="ansi-red-fg">../cache/jinja/__jinja2_e8c5fd6f2818e239fd5346c73f9f426dc69e894f.cache</span>
	<span class="ansi-red-fg">../cache/jinja/__jinja2_f10aeb879f232e45fd0c98d26fe4402689ed79bb.cache</span>
	<span class="ansi-red-fg">../cache/jinja/__jinja2_f9f7109b3bc5081e641cd485aefaf7bc634b5ee1.cache</span>
	<span class="ansi-red-fg">../cache/posts/binder-+-nikola-+-jupyter-+-github-blogging-resourceless.html</span>
	<span class="ansi-red-fg">../cache/posts/binder-+-nikola-+-jupyter-+-github-blogging-resourceless.html.dep</span>
	<span class="ansi-red-fg">../cache/posts/binder-+-nikola-+-jupyter-+-github-blogging-resourceless.html.es</span>
	<span class="ansi-red-fg">../cache/posts/binder-+-nikola-+-jupyter-+-github-blogging-resourceless.html.es.dep</span>
	<span class="ansi-red-fg">../output/2017/12/</span>
	<span class="ansi-red-fg">../output/categories/binder.html</span>
	<span class="ansi-red-fg">../output/categories/binder.xml</span>
	<span class="ansi-red-fg">../output/categories/github.html</span>
	<span class="ansi-red-fg">../output/categories/github.xml</span>
	<span class="ansi-red-fg">../output/es/2017/12/</span>
	<span class="ansi-red-fg">../output/es/categories/binder.html</span>
	<span class="ansi-red-fg">../output/es/categories/binder.xml</span>
	<span class="ansi-red-fg">../output/es/categories/github.html</span>
	<span class="ansi-red-fg">../output/es/categories/github.xml</span>
	<span class="ansi-red-fg">../output/es/posts/binder-+-nikola-+-jupyter-+-github-blogging-resourceless.html</span>
	<span class="ansi-red-fg">../output/es/posts/binder-+-nikola-+-jupyter-+-github-blogging-resourceless.ipynb</span>
	<span class="ansi-red-fg">../output/posts/binder-+-nikola-+-jupyter-+-github-blogging-resourceless.html</span>
	<span class="ansi-red-fg">../output/posts/binder-+-nikola-+-jupyter-+-github-blogging-resourceless.ipynb</span>
	<span class="ansi-red-fg">.ipynb_checkpoints/binder-+-nikola-+-jupyter-+-github-blogging-resourceless-checkpoint.ipynb</span>
...
<div class="inner_cell">
  <div class="input_area">
    <div class=" highlight hl-ipython3"><pre><span></span><span class="o">!</span>git add ../cache/*
</pre></div>
...
<div class="inner_cell">
  <div class="input_area">
    <div class=" highlight hl-ipython3"><pre><span></span><span class="o">!</span>git add ../output/*
</pre></div>
...
<div class="inner_cell">
  <div class="input_area">
    <div class=" highlight hl-ipython3"><pre><span></span><span class="o">!</span>git add binder-+-nikola-+-jupyter-+-github-blogging-resourceless.ipynb
</pre></div>
...
<div class="inner_cell">
  <div class="input_area">
    <div class=" highlight hl-ipython3"><pre><span></span><span class="o">!</span>git commit -m <span class="s2">&quot;Build the site in my binder instance&quot;</span>
</pre></div>
...
<div class="output_subarea output_stream output_stdout output_text">
<pre>[master 658c5e23] Build the site in my binder instance
...
<div class="inner_cell">
<div class="text_cell_render border-box-sizing rendered_html">
<p>And I can push the content (<code>git push origin master</code>) using the terminal as we did in a previous step.</p>
<h3 id="Visualize-your-site">Visualize your site<a class="anchor-link" href="#Visualize-your-site">&#182;</a></h3><p>This could be an issue because <strong>Nikola</strong> offers a <code>nikola serve</code> command to preview your site, but using it inside <strong>Binder</strong> is probably not trivial. And you can do a little trick to be able to explore the new content. You just need to deploy the <code>output</code> folder contaning the updated site under the same <code>gh-pages</code> branch from where you usually serve your site.</p>
<p>And you can do that quite easily with:</p>
<p><strong>Note</strong>: <code>git stash</code> is only needed here because I am executing git commands from this very same future blog post</p>
...
<div class="output_subarea output_stream output_stdout output_text">
<pre>Saved working directory and index state WIP on master: 658c5e23 Build the site in my binder instance
...
<div class="inner_cell">
  <div class="input_area">
    <div class=" highlight hl-ipython3"><pre><span></span><span class="o">!</span>git checkout gh-pages
</pre></div>
...
<div class="output_subarea output_stream output_stdout output_text">
<pre>Switched to branch &#39;gh-pages&#39;
...
<div class="inner_cell">
  <div class="input_area">
    <div class=" highlight hl-ipython3"><pre><span></span><span class="o">!</span>git checkout master -- ../output
</pre></div>
...
<div class="inner_cell">
  <div class="input_area">
    <div class=" highlight hl-ipython3"><pre><span></span><span class="o">!</span>git commit -m <span class="s2">&quot;Add output dir to gh-pages to pre-visualize the changes&quot;</span>
</pre></div>
...
<div class="output_subarea output_stream output_stdout output_text">
<pre>[gh-pages 4fb3a46f] Add output dir to gh-pages to pre-visualize the changes
...
<div class="inner_cell">
<div class="text_cell_render border-box-sizing rendered_html">
<p>And now, you just push the content into the <code>gh-pages</code> branch  with <code>git push origin gh-pages</code> (use the terminal as I indicarted before) and you can visit your <em>draft</em> site appeding <code>output</code> to the base url, in my case: <a href="http://www.damian.oquanta.info/output/">http://www.damian.oquanta.info/output/</a>.</p>
<p>If you are not happy with it, if it does not looks like right, just iterate with more changes! Otherwise you can go to the deploy step!</p>
<h2 id="Deploy-your-site">Deploy your site<a class="anchor-link" href="#Deploy-your-site">&#182;</a></h2><p>OK, given the previous steps, I guess you will figure out how to perform the next steps:</p>
<p><strong>NOTE</strong>: <code>rm</code> is only needed here because I am executing git commands from this very same future blog post</p>
...
<div class="inner_cell">
  <div class="input_area">
    <div class=" highlight hl-ipython3"><pre><span></span><span class="o">!</span>rm binder-+-nikola-+-jupyter-+-github-blogging-resourceless.ipynb 
</pre></div>
...
<div class="inner_cell">
  <div class="input_area">
    <div class=" highlight hl-ipython3"><pre><span></span><span class="o">!</span>git commit -m <span class="s2">&quot;Add new stuff to the blog post (mostly deploy section)&quot;</span>
</pre></div>
...
<div class="output_subarea output_stream output_stdout output_text">
<pre>[master 7927ad63] Add new stuff to the blog post (mostly deploy section)
...
<div class="inner_cell">
<div class="text_cell_render border-box-sizing rendered_html">
<p>In the terminal: <code>git push origin master</code> to persist the latest content... again. And use a subtree approach to split the <code>output</code> directory and put it as a new <code>gh-pages</code> branch.</p>
<p><strong>Note</strong>: First delete the existing <code>gh-pages</code> branch to avoid conflicts and <code>cd ..</code> because you need to subtree from the top directory.</p>
...
<div class="inner_cell">
  <div class="input_area">
    <div class=" highlight hl-ipython3"><pre><span></span><span class="o">!</span>git branch -d gh-pages
</pre></div>
...
<div class="output_subarea output_stream output_stdout output_text">
<pre>warning: deleting branch &#39;gh-pages&#39; that has been merged to
...
<div class="inner_cell">
  <div class="input_area">
    <div class=" highlight hl-ipython3"><pre><span></span><span class="o">!</span>git subtree split --prefix output -b gh-pages
</pre></div>
...
<div class="inner_cell">
<div class="text_cell_render border-box-sizing rendered_html">
<p>Finally, in the terminal: <code>git push -f origin gh-pages:gh-pages</code> to deploy your site and remove the local <code>gh-pages</code> with <code>git branch -D gh-pages</code>.</p>
<p>And you are done!!!</p>
<p><strong>Note</strong>: As you probably realized, you can run all the commands I described from the terminal and use the notebook just to write the blog posts. That would be, probably, the most common pattern. But this post in nicer and more explanatory if I run most of the commands here ;-)</p>
<h3 id="Conclusion">Conclusion<a class="anchor-link" href="#Conclusion">&#182;</a></h3><p>You have your new post <em>live</em>, in a <em>resourceless</em> sort of workflow, thanks to the <strong>Jupyter</strong> ecosystem, <strong>Nikola</strong> and <strong>Github</strong>.</p>
<p>OK, long enough post. Sorry about that, but I need to give you the complete workflow if you want/need to use it.</p>
<p>Happy coding!</p>
<p>And blogging!</p>
